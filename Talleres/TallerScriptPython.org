...
** Solución Planteada
*** Archivo de funciones projectile_functions.py
...
** SOLVE_IVP
~solve_ivp~ es una función del paquete ~scipy.integrate~ que resuelve
...
** ARGPARSE
~argparse~ es un módulo estandar de Python para gestionar argumentos desde
...
poder ingresar la resistencia del aire ~B~, velocidad inicial ~V~, tiempo
de vuelo ~t~ y los ángulos de lanzamiento.
** NUMPY
~numpy~ es una librería Python para el cálculo numérico. Lo hemos implementado
para el apartado de calculos como: ~cos~ y ~sin~ de los ángulos, la raíz
cuadrada ~sqrt~ de la velocidad, el valor de ~pi~ oara convertir grados a
radianes, y ~linspace~ para generar el vector de tiempos.

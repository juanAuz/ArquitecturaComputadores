...
\addbibresource{.././FormatoTareas/bibliography.bib}
...
siguientes preguntas considerando la Tabla \ref{tab:orgd4e5da7} como el
mapa de Memoria de la máquina y obtenga el programa en lenguaje ensamblador y en
RTL. 

\begin{table}[htbp]
\caption{\label{tab:orgd4e5da7}Mapa de Memoria}
\centering
\begin{tabular}{rrrrr}
\hline
...
\([IR] \leftarrow 01\ 940\)\\
\end{verse}
\subsection{0x01 \(\to\) {[}AC] \(\leftarrow\) {[}X]}
\label{sec:org454752c}

\begin{verse}
\([AC] \leftarrow [940]\)\\
\([AC] \leftarrow 000000005\)\\
\([PC] \leftarrow 0x300\ (der)\)\\
\([IR] \leftarrow [PC]\)\\
\([IR] \leftarrow 06\ 941\)\\
\end{verse}
\subsection{0x06 \(\to\) {[}AC] \(\leftarrow\) {[}AC] - [X]}
\label{sec:orgb4418d2}

\begin{verse}
\([AC] \leftarrow [AC] - [941]\)\\
\([AC] \leftarrow 000000005 - 000000002\)\\
\([AC] \leftarrow 000000003\)\\
\(Carry = 1\)\\
\([PC] \leftarrow 0x301\)\\
\([IR] \leftarrow [PC]\)\\
\([IR] \leftarrow 21\ 940\)\\
\end{verse}
\subsection{0x21 [X] \(\leftarrow\) {[}AC]}
\label{sec:org2679b63}

\begin{verse}
\([940] \leftarrow [AC]\)\\
\([940] \leftarrow 000000003\)\\
\([PC] \leftarrow 0x301\ (der)\)\\
\([IR] \leftarrow [PC]\)\\
\([IR] \leftarrow 14\ 000\)\\
\end{verse}
\subsection{0x14 [AC] \(\leftarrow\) {[}AC] \texttimes{} 2}
\label{sec:orgef42f62}

\begin{verse}
\([AC] \leftarrow [AC] \times 2\)\\
\([AC] \leftarrow 000000006\)\\
\([PC] \leftarrow 0x302\)\\
\([IR] \leftarrow [PC]\)\\
\([IR] \leftarrow 21\ 941\)\\
\end{verse}
\subsection{0x21 [X] \(\leftarrow\) {[}AC]}
\label{sec:org545b5e6}

\begin{verse}
\([941] \leftarrow [AC]\)\\
\([941] \leftarrow 000000006\)\\
\([PC] \leftarrow 0x302\ (der)\)\\
\([IR] \leftarrow [PC]\)\\
\([IR] \leftarrow 99\ 000\)\\
\(Fin \to Estado\ Halt\)\\
\end{verse}
